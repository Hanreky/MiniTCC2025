...
public class ConClientes {
    ...
    public void cadastrar(Clientes cliente) {
        String sql = "INSERT INTO TBCLIENTE(NOMECLIENTE, CELCLIENTE, CPFCLIENTE, EMAILCLIENTE, STATUSCLIENTE) "
                + "VALUES (?, ?, ?, ?, ?)";
    ...
    public Clientes pesquisarCPF(String cpf) {
        String sql = "SELECT * "
                + "FROM TBCLIENTE "
                + "WHERE CPFCLIENTE = ? && STATUSCLIENTE = 1";
        try {
    ...
    public boolean verificarCpf(String cpf) {
        ...
        try {
            ...
            while (rs.next()) {
                if(rs.getString("CPFCLIENTE").equals(cpf)){
                    return true;
                }                
            }
            return false;
